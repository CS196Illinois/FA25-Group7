        ...
        for i in range(0, len(event_listings)):
            ...
            for key in details:
                match key:
                    ...
                    case "sponsor":
                        event_info["organizer"] = {"displayName": details[key]}
...
def scrape_state_farm():
    ...
    with sync_playwright() as p:
        ...
        for i in range(0, len(event_listings)):
            ...
            event_info["organizer"] = {"displayName": "State Farm Center"}
...
def scrape_athletics():
    ...
    for calendar_link in ATHLETIC_TICKET_LINKS:
        ...
        for i in range(0, len(event_listings)):
            ...
            event_info["organizer"] = {"displayName": "Fighting Illini Athletics"}
...
def run_scraper():
    ...
    # Get all existing entries BEFORE we insert the new entry
    all_entries = ref.get()

    # Save new entry to Realtime Database with timestamp
    new_entry = ref.push({
        'data': scraped_data,
        'created_at': datetime.utcnow().isoformat()
    })

    print(f"✅ Data saved to Firebase Realtime Database! Key: {new_entry.key}")

    # Delete all old entries to save space (keep only the latest one)
    if all_entries:
        print(f"🧹 Cleaning up old entries...")
        for old_key in all_entries.keys():
            ref.child(old_key).delete()
            print(f"   Deleted old entry: {old_key}")
        print(f"✅ Cleanup complete! Only latest entry remains.")
#-----------------------LOCAL TESTS-----------------------#
